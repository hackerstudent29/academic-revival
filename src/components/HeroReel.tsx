        ...
        {[...items, ...items].map((item, i) => (
          ...
          >
            ...
            <div className="absolute inset-x-6 bottom-6 flex flex-col gap-2 translate-y-4 opacity-0 transition-all duration-500 ease-out group-hover:translate-y-0 group-hover:opacity-100">
              <span className="inline-flex self-start rounded-[3px] bg-primary px-3 py-1 text-[11px] font-black uppercase tracking-[0.15em] text-primary-foreground">
                {item.tag}
              </span>
              <h3 className="text-3xl font-black leading-tight tracking-tight text-white drop-shadow-[0_2px_20px_rgba(0,0,0,0.7)] sm:text-4xl">
                {item.title}
              </h3>
              <p className="max-w-[85%] text-sm leading-relaxed text-white/90">{item.desc}</p>
            </div>
          </div>
        ))}
      </div>
